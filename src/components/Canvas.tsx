...
export default function Canvas({
  ...
  const calculateGridPosition = useCallback((dropX: number, dropY: number, containerWidth: number): GridPosition => {
    const gridWidth = containerWidth / GRID_COLUMNS;
    const gridX = Math.max(0, Math.min(GRID_COLUMNS - 1, Math.floor(dropX / gridWidth)));
  ...
  const handleComponentReorder = useCallback((e: React.DragEvent) => {
    ...
    const { x: gridX, y: gridY } = calculateGridPosition(dropX, dropY, rect.width);
  ...
  const handleResize = useCallback((id: string, newWidth: number) => {
  ...
  const renderComponent = useCallback((component: ComponentType) => {
    const commonProps = {
      onUpdate: onUpdateComponent,
      onUpdateResize: onUpdateComponentResize,
      onResize: (width: number) => handleResize(component.id, width),
  ...
  const maxY = components.length > 0 ? Math.max(...components.map((c) => c.position.y)) : 0;
  ...
  return (
    ...
    >
      {components.length === 0 ? (
        ...
        <div
          className={`grid grid-cols-12 w-full ${!isEditing ? 'gap-4 p-6' : ''}`}
          style={{
            gridTemplateRows: `repeat(${Math.max(gridRows, 1)}, minmax(100px, auto))`,
          }}
        >
          {gridItems}
        </div>
